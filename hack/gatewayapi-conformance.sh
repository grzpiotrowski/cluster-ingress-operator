...
# Test all standard features except GatewayStaticAddresses, which requires a
# pre-allocated IP address.
...
go test ./conformance -v -timeout 2h -run TestConformance -args "--supported-features=${SUPPORTED_FEATURES}" "--gateway-class=${GATEWAYCLASS_NAME}"
